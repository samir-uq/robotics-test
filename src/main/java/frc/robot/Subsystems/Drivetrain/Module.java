...
/// A MODULE refers to like each wheel of the drivetrain

public class Module implements IModuleIO, Subsystem, Sendable {
    private static int numModTracker = 0;

    /// the drive motor hardware setup
    /// has pid, encoder and configs
    SparkFlex driveMotor;
    SparkFlexConfig driveConfig;
    SparkClosedLoopController drivePID;
    RelativeEncoder driveEncoder;

    /// the turn motor hardware setup
    /// has pid, encoder, configs
    /// our current drivetrain use cancoder for absolute encoder
    SparkMax turnMotor;
    ...
    public Module(
        ...
        driveConfig.closedLoop
                ...
                .pidf(Constants.Module.drivePIDGains[0], Constants.Module.drivePIDGains[1],
                        Constants.Module.drivePIDGains[2], Constants.Module.drivePIDGains[3])
        ...
        turnConfig.closedLoop
                .feedbackSensor(FeedbackSensor.kPrimaryEncoder)
                .pidf(Constants.Module.turnPIDGains[0], Constants.Module.turnPIDGains[1],
                        Constants.Module.turnPIDGains[2], Constants.Module.turnPIDGains[3])
    ...
    /// called periodic to update states and update the motors so they have the right speed and rotation.
    @Override
    ...
    public ModuleStatesAutoLogged updateStates(ModuleStatesAutoLogged _states) {
        ...
        // _states.currentAngleDeg = Math.abs(newAngle - _states.currentAngleDeg) / .02
        // > 1000 ? _states.currentAngleDeg : newAngle;
        _states.currentSpeedMpS = driveEncoder.getVelocity();
        // _states.currentDistanceM = Math.abs(newDistance - _states.currentDistanceM) /
        // .02 > 25 ? _states.currentDistanceM : newDistance;
    ...
    /// below here are again, setters and getters
    /// most of the syntax is self explainable.
    /// as said before, you just look at code and dcoumentation to learn
    /// other .java files in this codebase explain lot of this stuff like ControlType and whatnot
    /// but most of it is just simple code that is easy to grasp onto.
    ...
    /// this just adds data of the module to the robot
